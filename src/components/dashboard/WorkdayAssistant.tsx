...
export default function WorkdayAssistant({ entries, rules }: WorkdayAssistantProps) {
  ...
  return (
    <section className="glass-card p-4 md:p-5 animate-in stagger-1">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-semibold">Jobber du hjemme i dag?</p>
          <p className="mt-1 text-xs" style={{ color: "var(--fg-muted)" }}>
            Ingen arbeidsdag er logget. Forslaget blir {customStart}-{suggestedEnd}, {formatDuration(suggestedDuration)} netto.
          </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            aria-label="Starttid"
          />
          <button className="btn-primary h-10 px-4" onClick={() => createWorkday(customStart)} disabled={createEntry.isPending}>
            Logg dagen
          </button>
          <button className="btn-secondary h-10 px-4" onClick={() => createWorkday(formatDateTime(now))} disabled={createEntry.isPending}>
            Start nå
          </button>
          <button className="h-10 px-2 text-xs" style={{ color: "var(--fg-faint)" }} onClick={dismiss}>
            Ikke i dag
          </button>
        </div>
      </div>
